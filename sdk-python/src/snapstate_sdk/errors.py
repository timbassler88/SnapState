"""
Typed exceptions for the SnapState SDK.

All exceptions inherit from SnapStateError so callers can catch the base
class broadly or individual subclasses for fine-grained handling.
"""


class SnapStateError(Exception):
    """Base exception for all SnapState SDK errors."""
...
CheckpointError = SnapStateError  # deprecated alias


class AuthError(SnapStateError):
    """Raised on 401 Unauthorized — invalid or missing API key / JWT."""
    pass


class NotFoundError(SnapStateError):
    """Raised on 404 Not Found."""
    pass


class ConflictError(SnapStateError):
    """Raised on 409 Conflict (e.g. ETag mismatch)."""
    pass


class RateLimitError(SnapStateError):
...
class ValidationError(SnapStateError):
    """Raised on 400 Bad Request — invalid input data."""
    pass


class PayloadTooLargeError(SnapStateError):

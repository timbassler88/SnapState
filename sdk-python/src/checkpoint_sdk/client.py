...
Context manager (sync):
    with CheckpointClient(api_key="cpk_...") as client:
        client.save(...)

Context manager (async):
    async with CheckpointClient(api_key="cpk_...") as client:
...
class CheckpointClient:
    """
    Client for the Checkpoint Service API.

    Parameters
    ----------
    api_key : str
        Your API key (begins with ``cpk_``).
    base_url : str
        Base URL for the Checkpoint Service. Defaults to ``http://localhost:3000``.
    timeout : float
    ...
    def _handle_response(self, response: httpx.Response) -> None:
        ...
        if status == 401:
            ...
            raise CheckpointError(message, status, code)
